...
@application.route('/api/v1/dokter')
def api_dokter():
    output_db = getMethod("SELECT * FROM `dokter`")
    dokter = {}
    dokter['dokter'] = []
    for column in output_db:
        content = {'id_dokter':column[0], 'nama_dokter':column[1], 'id_spesialis':column[2], 'no_telp':column[3], 'alamat':column[4], 'pagi':column[5]}
        dokter['dokter'].append(content)
        content = {}
...
def readDokter():
    output = getData(f"http://127.0.0.1:5000/api/v1/dokter")
    flash('Data dokter berhasil didapatkan!', 'success')
    return render_template('dokter.html', data=output['dokter'])
...
def updateDokter():
        ...
        pagi = request.form['pagi']
        cursor = mysql.connection.cursor()
        cursor.execute("UPDATE dokter SET nama_dokter=%s,id_spesialis=%s,no_telp=%s,alamat=%s,pagi=%s WHERE id_dokter=%s", (nama_dokter,id_spesialis,no_telp,alamat,pagi,id_dokter,))
        mysql.connection.commit()
        return redirect(url_for('dokter'))
